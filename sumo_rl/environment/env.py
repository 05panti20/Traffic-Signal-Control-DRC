...
class SumoEnvironment(gym.Env): 
    ...
    def __init__(
        ...
        print_step_progress: bool = False,
        ...
        # If True, print an in-place updating progress line for each SUMO step
        self.print_step_progress = print_step_progress
    ...
    def _sumo_step(self):
        ...
        self.num_arrived_vehicles += self.sumo.simulation.getArrivedNumber() #この時間ステップに目的地に到達し、道路ネットワークから削除された車両の数。
        self.num_departed_vehicles += self.sumo.simulation.getDepartedNumber() #この時間ステップで道路ネットワークに挿入された車両の数。
        self.num_teleported_vehicles += self.sumo.simulation.getEndingTeleportNumber() #この時間ステップでテレポートが終了した車両の数。
        vehicles = self.sumo.vehicle.getIDList()
        for v in vehicles:
            self.co2_emission += self.sumo.vehicle.getCO2Emission(v)  # 累積CO2排出量を更新
        # Optional in-place progress print (updates the same console line)
        if getattr(self, "print_step_progress", False):
            try:
                vehicles = self.sumo.vehicle.getIDList()
                pending = len(self.sumo.simulation.getPendingVehicles()) if hasattr(self.sumo.simulation, 'getPendingVehicles') else 0
                # Use carriage return to overwrite the same line.
                import sys

                sys.stdout.write(f"Step #{self.sim_step:.2f} (vehicles TOT {len(vehicles)} BUF {pending})\r")
                sys.stdout.flush()
            except Exception:
                # Avoid raising from logging-related failures
                pass

    def _get_system_info(self):
        vehicles = self.sumo.vehicle.getIDList()
        speeds = [self.sumo.vehicle.getSpeed(vehicle) for vehicle in vehicles]
        step_waiting_times = [self.sumo.vehicle.getWaitingTime(vehicle) for vehicle in vehicles]
        ...
        return {
            ...
            "system_total_co2_emission": self.co2_emission,
            "system_total_waiting_time": sum(step_waiting_times),
            "system_mean_waiting_time": 0.0 if len(vehicles) == 0 else np.mean(step_waiting_times),
