...
class TrafficSignal:
    ...
    def _observation_fn_default(self):
        ...
        veh_route_one_hot_distance = self.get_veh_routeOneHot_Distance()
    ...
    def _get_all_veh_list(self):
        veh_list = self.sumo.vehicle.getIDList()
        return veh_list
        
    
    # def get_veh_position(self):
    #     """Returns the position of vehicles in the intersection."""
    #     veh_ids = self._get_veh_list()
    #     veh_positions = []
    #     for veh in veh_ids:
    #         pos = self.sumo.vehicle.getPosition(veh)
    #         veh_positions.append(pos)
    #     return veh_positions
    
    # def get_veh_lane_position(self):
    #     """Returns the lane and position of vehicles in the intersection."""
    #     veh_ids = self._get_veh_list()
    #     veh_lane_positions = []
    #     for veh in veh_ids:
    #         pos = self.sumo.vehicle.getLanePosition(veh)
    #         pos = int(pos)
    #         veh_ID = float(veh)
    #         veh_ID = int(veh_ID)
    #         veh_lane_positions.append(veh_ID)
    #         veh_lane_positions.append(pos)
    #     if len(veh_lane_positions) < 180:
    #         veh_lane_positions += [0] * (180 - len(veh_lane_positions))
    #     elif len(veh_lane_positions) > 180:
    #         veh_lane_positions = veh_lane_positions[:180]
    #     return veh_lane_positions
    
    def get_veh_routeOneHot_Distance(self):
        """Returns the route one-hot and up-to-5 lane-positions per route.

        Simplified implementation:
        - Uses a lookup map for (origin_char, dest_token) -> route name
        - Collects positions per route in a dict (safer than fixed-index lists)
        - Keeps output shape identical to original: for each of 12 routes
          append 12-dim one-hot then top-5 normalized positions (pad with 0.0)
        """
        from collections import defaultdict

        veh_ids = self._get_all_veh_list()

        # ルート名の順序は既存実装と同じ
        route_names = [
            "route_ws", "route_we", "route_wn",
            "route_se", "route_sn", "route_sw",
            "route_en", "route_es", "route_ew",
            "route_ns", "route_ne", "route_nw",
        ]
        route_index = {name: i for i, name in enumerate(route_names)}

        # (origin_char, dest_token) -> route_name
        route_lookup = {
            ("w", "t_s"): "route_ws", ("w", "t_e"): "route_we", ("w", "t_n"): "route_wn",
            ("s", "t_e"): "route_se", ("s", "t_n"): "route_sn", ("s", "t_w"): "route_sw",
            ("e", "t_n"): "route_en", ("e", "t_s"): "route_es", ("e", "t_w"): "route_ew",
            ("n", "t_s"): "route_ns", ("n", "t_e"): "route_ne", ("n", "t_w"): "route_nw",
        }

        pos_per_route = defaultdict(list)

        for veh in veh_ids:
            try:
                route_info = self.sumo.vehicle.getRoute(veh)
                # route_info might be a sequence; original code accessed [0] and [2]
                key = (route_info[0], route_info[2])
            except Exception:
                # If route parsing fails, skip this vehicle
                continue
            ...
            try:
                pos = int(self.sumo.vehicle.getDistance(veh)) / 200.0  # normalize
            except Exception:
                # If lane position not available, skip
                continue
            
            if pos < 1:
                pos_per_route[route].append(pos)

        veh_routenum_lanepositions: List[float] = []
        for i, route in enumerate(route_names):
            # ルートのワンホット（インデックスを使って効率的に作る）
            one_hot = [0] * len(route_names)
            one_hot[i] = 1
            veh_routenum_lanepositions.extend(one_hot)

            # 位置は大きい順に上位5件、足りなければ0で埋める
            top5 = sorted(pos_per_route.get(route, []), reverse=True)[:5]
            if len(top5) < 5:
                top5 += [0.0] * (5 - len(top5))
            veh_routenum_lanepositions.extend(top5)

        return veh_routenum_lanepositions
    
    def get_co2_emission(self) -> float:
        """Returns the total CO2 emission of vehicles in the intersection."""
        total_co2 = 0.0
        vehs = self._get_veh_list()
        for v in vehs:
            total_co2 += self.sumo.vehicle.getCO2Emission(v)
        return total_co2
